...
class Organization {
  ...
  info() {
    ...
    return co(function* () {
      ...
      const likeRegex = /(?:Like|いいね)/i
      const likeStat  = stats.find(stat =>  likeRegex.test(stat.label))
      const postStat  = stats.find(stat => !likeRegex.test(stat.label))

      return {
        like     : parseInt(likeStat.value, 10),
